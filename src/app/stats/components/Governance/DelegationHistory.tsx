...
import {
  DelegationHistoryGraph,
  DelegationHistoryGraphRow,
} from "./DelegationHistoryGraph"

export const DelegationHistory = async () => {
  const { mobileEpochs, iotEpochs } = await fetchRecentEpochs()

  const cleanedData: DelegationHistoryGraphRow[] = mobileEpochs
    .map((mEpoch, index) => {
      ...
      const percentIot = (iotDelegated / (mobileDelegated + iotDelegated)) * 100
      return {
        iotDelegated,
        mobileDelegated,
        percentIot,
